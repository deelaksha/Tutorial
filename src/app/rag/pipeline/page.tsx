...
const DIAGRAM = {
  ...
  nodes: [
    ...
    { id: "format", icon: "=Ý", label: "format_docs", sub: "join chunks", x: 47, y: 50, color: "#fbbf24" },
    { id: "prompt", icon: "=¬", label: "Prompt", sub: "system + context + question", x: 58, y: 50, color: "#f472b6" },
    { id: "llm", icon: ">à", label: "gpt-4o-mini", sub: "generate", x: 69, y: 50, color: "#60a5fa" },
    { id: "answer", icon: "", label: "Answer", sub: "with citations", x: 80, y: 50, color: "#34d399" },
    { id: "sources", icon: "=Ú", label: "Sources", sub: "metadata list", x: 91, y: 50, color: "#a78bfa" },
  ],
  edges: [
...
export default function PipelinePage() {
  return (
    <TopicShell
      ...
      nav={NAV}
      badges={["= LCEL chain: retriever | prompt | LLM", "=¬ RAG prompt design", "=Ú Citations + sources", ">ê Quick eval harness", "> Complete chatbot script"]}
...
Sources: docs/manual.md, docs/faq.md`}
        />
        <Callout type="note">
          =Ì <strong>Why retrieval isn&apos;t streamed</strong>: Retrieval is fast (~50-100ms for Chroma). Streaming retrieval adds complexity for no UX gain. Stream only the LLM output (1-3 seconds for a 50-token answer). The user sees: [retrieval happens] ’ [answer starts streaming immediately].
        </Callout>
      </Section>
